...
export class AuthService {
  ...
  async userLogout(id: string, token: string): Promise<void> {
    let user = await this.usersService.findUserById(id);
    token = token.split(" ")[1]
    if (Array.isArray(user.tokens)) {
      user.tokens = user.tokens?.filter((item:string) => item!==token);
    }
    await this.usersService.updateUser(user)
  }

  async verifyUserLogin(id: string,token:string): Promise<VerifyUserInterface> {
    const user = await this.usersService.findUserById(id);
    if (!user || user.login_attempts > 5 || !user.tokens) throw new HttpException(ERROR_MESSAGES.USERS.USER_NOT_FOUND, HttpStatus.UNAUTHORIZED); 
    if(Array.isArray(user.tokens) && isEmpty(user.tokens?.find(item=>item===token))) throw new HttpException(ERROR_MESSAGES.USERS.USER_NOT_FOUND, HttpStatus.UNAUTHORIZED);
